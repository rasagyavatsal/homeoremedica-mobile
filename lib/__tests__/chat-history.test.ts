...
const firestoreMocks = firestore as unknown as {
  addDoc: jest.Mock;
...
describe('loadChat', () => {
  it('returns the chat record when it exists', async () => {
    const timestamp = new firestoreMocks.Timestamp(1000, 0);
    firestoreMocks.doc.mockReturnValue({ __docRef: 'chats/chat-1' });
...
describe('subscribeToChats', () => {
  ...
  it('queries newest-first for the user and maps snapshots to summaries', () => {
    ...
    const handleSnapshot = firestoreMocks.onSnapshot.mock.calls[0]?.[1] as (
      snapshot: {
        docs: { id: string; data: () => { title?: unknown; updatedAt?: unknown } }[];
      }
    ) => void;
    handleSnapshot({
      docs: [
  ...
  it('returns the unsubscribe function', () => {
    ...
    expect(subscribeToChats('user-1', jest.fn())).toBe(unsubscribe);
  });
});
